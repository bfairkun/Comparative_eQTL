...
for (i in seq_along(TissueList)){
  ...
  SamplesToSubset <- SampleList %>% filter(SMTSD == TissueList[i] & SAMPID %in% colnames(CountTable)) %>% pull(SAMPID)
  CountTableSubset <- CountTable %>%
    dplyr::select(Name, SamplesToSubset) %>%
    column_to_rownames("Name")
  ParameterEstimates <- GetParameterEstimatesOfUnderlyingGamma_lengthAdjusted_FromTable(CountTableSubset, rep(1, length(GeneLengthVector)))
