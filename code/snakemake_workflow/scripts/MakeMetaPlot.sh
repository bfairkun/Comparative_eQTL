...
awk -F'\t' -v OFS='\t' 'NR>1 && $2!=23 {print $2,$3,$3+1,0.01}' ../eQTL_mapping/MatrixEQTL/ForAssociationTesting.snploc | sort -k1,1 -k2,2n | awk -v OFS='\t' 'BEGIN {print "track type=bedGraph name=track_label description=center_label"} {print $0}' > ../scratch/test.snps.bg
