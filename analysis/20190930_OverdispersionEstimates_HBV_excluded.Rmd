...
gseDO.mean <-gseDO(sort(MeanOverdispersion, decreasing=T), keyType='ENSEMBL')
...
CorShrunkODM <- ToPlot %>% rownames_to_column() %>%
  dplyr::select(rowname, Chimp.Overdispersion, Human.Overdispersion) %>% 
  left_join(OverdispersionTable, by=c("rowname"="gene")) %>%
  column_to_rownames() %>%
  as.matrix() %>% scale() %>% CorShrinkData(sd_boot = FALSE, image = "both",
                     image.control = list(tl.cex = 0.2))  

#After using CorShrink
OverdispersionCorrelationHeatmap2 <- heatmap.2(CorShrunkODM$cor, trace="none", dendrogram=c("col"), labCol=F, cexRow = 0.7, ColSideColors=GtexScheme, colRow = GtexScheme, adjRow=c(0,0), offsetRow=-0.6, main="overdispersion correlation matrix") 
...
```{r, eval=F}
#block of code to make quick plots for lab updates
LoessPlot <- ggplot(ToPlot, aes(x=Chimp.Mean.Expression, y=log10(Chimp.Overdispersion))) +
  geom_point(alpha=0.1) +
  scale_x_continuous(name="log(Mean expression)") +
  scale_y_continuous(name="log(Overdispersion)", limits=c(-2.5,1.5)) +
  geom_smooth(method=loess, show.legend = FALSE, se=T, method.args=list(degree=1)) +
  theme_bw() +
...
ggplot(ToPlot, aes(x=Chimp.Mean.Expression, y=log10(Chimp.Residual))) +
  geom_point(alpha=0.1) +
  scale_x_continuous(name="log(Mean expression)") +
  scale_y_continuous(name="Residual overdispersion", limits=c(-1.5,1.5)) +
...
cor.test(log(ToPlot$Chimp.Residual),log(ToPlot$Human.Residual), use="complete.obs")
lb1 <- paste("~R^2==~", round(R**2,2))
Overdispersion.Species.Correlation.Corrected <- ggplot(ToPlot, aes(x=log10(Chimp.Residual), y=log10(Human.Residual))) +
  xlim(c(-1.5,1.5)) +
  ylim(c(-1.5,1.5)) +
  xlab(TeX("Chimp_{OverdispersionResidual}")) +
  ylab(TeX("Human_{OverdispersionResidual}")) +
  geom_point(alpha=0.05) +
  annotate("text",x=Inf,y=-Inf, label=lb1, hjust=1, vjust=-1, parse=TRUE, size=4) +
  theme_bw() +
  theme(axis.text.x=element_blank(), axis.ticks.x=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank(), rect = element_rect(fill = "transparent"),
        panel.background = element_rect(fill = 'transparent', colour = 'transparent'),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
  ...
  ggplot(Essentiality, aes(color=pLI.Score.essentiality,x=log10(Human.Residual))) +
    stat_ecdf(geom = "step") +
    ylab("Cumulative frequency") +
    xlab("Residual Overdispersion") +
    xlim(c(-0.75,0.75)) +
    annotate("text", vjust=-1, hjust=1, x = Inf, y = -Inf, label = paste("P =", signif(wilcox.test(data=Essentiality, Human.Residual ~ pLI.Score.essentiality, alternative="less")$p.value, 2) )) +
    theme_bw() +
    theme(legend.position="bottom")

ToPlot$AvgResidOverdisp <- (log(ToPlot$Chimp.Residual) + log(ToPlot$Human.Residual))/2

ToPlot %>%
  rownames_to_column("ENSEMBL_ID") %>%
  mutate(Symbol=mapIds(org.Hs.eg.db, keys=ENSEMBL_ID, column="SYMBOL", keytype="ENSEMBL", multiVals="first")) %>% dplyr::select(ENSEMBL_ID, Symbol, AvgResidOverdisp, Chimp.Mean.Log2RPKM) %>%
arrange(desc()) %>% write.table("~/Desktop/OverdispersedOrderedGenelist.tsv", sep='\t', row.names = F, quote=F)
