...
gseaplot2(gsego.cc.chimp.high.var, geneSetID = c("GO:0002263", "GO:0061061"), title="ChimpVariance - HumanVariance", pvalue_table = TRUE)
...
A %>% as.data.frame() %>% filter(ONTOLOGY=="MF") %>% dplyr::select(Description)
...
Lets sort reverse and do GSEA

```{r}
gsego.all.human.high.var <- gseGO(gene = (sort(SortedGeneList)*-1),
                 OrgDb = org.Hs.eg.db,
                 keyType = 'ENSEMBL',
                 maxGSSize = 500,
                 ont = "ALL",
                 nPerm = 100000)
A<-as.data.frame(gsego.all.human.high.var)
A %>% as.data.frame() %>% filter(ONTOLOGY=="MF") %>% dplyr::select(Description)
bp2 <- simplify(gsego.all.human.high.var, cutoff=0.7, by="p.adjust", select_fun=min)
```
