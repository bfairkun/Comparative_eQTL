...
HighDispersionGene <- ChimpOverdispersionTable %>%
  ...
  filter(DifferenceInDispersion < 0.1) %>% head()
...
Colplots <- ChimpOverdispersionTable %>%
  dplyr::select(Chimp.Dispersion, Human.Dispersion, hgnc_symbol, gene) %>%
  filter(gene %in% GenesToPlot) %>%
  gather(key="sample", value="dispersion", -hgnc_symbol, -gene) %>% head()
...
ggsave("../figures/OriginalArt/Dispersion.chimp_human_example_genes.pdf", units="mm", height=100, width=100)
```
...
lb1 = paste0('atop(R^2==', signif(Correlation$estimate**2,2), ', P', format.pval(Correlation$p.value), ')')
...
lb1 = paste0('atop(R^2==', signif(Correlation$estimate**2,2), ', P', format.pval(Correlation$p.value, 2), ')')
...
CombinedTable %>%
  ggplot(aes(x=tau, y=DifferenceInDispersion)) +
    ...
    ylim(c(-3,3)) +
    xlab(TeX("Cell type specificity, $\\tau$")) +
    ylab(TeX("Dispersion_{Chimp} - Dispersion_{Human}")) +
    scale_fill_viridis_c(option="C") +
    annotate("text",x=Inf,y=-Inf, label=lb1, hjust=1, vjust=-0.1, parse=TRUE) +
    theme_bw() +
    theme(aspect.ratio = 0.8)
